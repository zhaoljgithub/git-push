...
class MCPHandler {
  ...
  async processNaturalLanguage(text, context = {}) {
    try {
      this.logger.info('处理自然语言请求', { text, context, service: 'git-push-mcp' });
      
      // 解析自然语言命令
      const parsedCommand = this.nlpProcessor.parseCommand(text);
      this.logger.info('命令解析结果', { parsedCommand, service: 'git-push-mcp' });
      
      // 根据命令类型处理
      switch (parsedCommand.command) {
        case 'commit':
          return await this.handleCommit(parsedCommand, context);
        case 'add':
          return await this.handleAdd(parsedCommand, context);
        case 'status':
          return await this.handleStatus(parsedCommand, context);
        case 'log':
          return await this.handleLog(parsedCommand, context);
        case 'diff':
          return await this.handleDiff(parsedCommand, context);
        case 'branch':
          return await this.handleBranch(parsedCommand, context);
        case 'push':
          return await this.handlePush(parsedCommand, context);
        default:
          return {
            success: false,
  ...
  async handleCommit(parsedCommand, context) {
    try {
      ...
      const autoStage = context.autoStage !== false; // 默认为true
      const autoPush = context.autoPush === true;   // 默认为false
      const conventionalCommits = context.conventionalCommits !== false; // 默认为true
      
      this.logger.info('开始处理提交操作', { 
        message, 
        autoStage, 
        autoPush, 
        conventionalCommits,
      ...
      // 检查是否有更改需要提交
      const hasChanges = (status.not_added && status.not_added.length > 0) || 
                        (status.modified && status.modified.length > 0) || 
                        (status.deleted && status.deleted.length > 0);
      
      if (!hasChanges && status.staged.length === 0) {
        return {
          success: false,
          error: '没有需要提交的更改',
      ...
      if (autoStage && hasChanges) {
        this.logger.info('自动暂存文件', { service: 'git-push-mcp' });
      ...
      // 格式化提交消息
      const finalMessage = conventionalCommits 
        ? this.nlpProcessor.formatCommitMessage(parsedCommand, true)
        : message;

      this.logger.info('准备提交', { 
  ...
  async commitChanges(message, files = [], stageAll = true, autoPush = false) {
    try {
      ...
      return {
        ...
        details: result.data,
        stats: result.stats,
        pushed: autoPush ? (result.pushed || pushResult?.success) : false,
        service: 'git-push-mcp'
      };
    } catch (error) {
   ...
   * 处理推送操作
   */
  async handlePush(parsedCommand, context) {
    try {
      const { message } = parsedCommand;
      const branch = message || null;
      ...
      const result = await this.gitOperator.push('origin', branch);
