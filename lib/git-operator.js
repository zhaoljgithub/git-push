...
class GitOperator {
  ...
  async push(remote = 'origin', branch, options = {}) {
    try {
      ...
      if (branch) {
        ...
        this.logger.info(`自动检测到当前分支: ${currentBranch}`, { service: 'git-push-mcp' });
        result = await this.git.push(remote, currentBranch, pushOptions);
      }
  ...
  async commitAndPush(message, options = {}) {
    try {
      ...
      // 先提交
      const commitResult = await this.commit(message, options);
      ...
      // 然后推送
      const pushResult = await this.push('origin', currentBranch, options);
      ...
      return {
        ...
        message: `${commitResult.message} 并推送至 ${currentBranch} 分支`
      };
    } catch (error) {
